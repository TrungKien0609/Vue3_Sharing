<template>
  <div class="container">
    ...
    <AddOrUpdateEmployeeForm v-if="isShowForm" :isAddOrUpdate="isAddOrUpdate"
      v-model:currentInputEmployeeName="currentEmployeeName" v-model:currentInputEmployeeEmail="currentEmployeeEmail"
      v-model:currentInputEmployeeCountry="currentEmployeeCountry" v-model:currentInputEmployeeAge="currentEmployeeAge"
      v-model:currentInputEmployeeSalary="currentEmployeeSalary" @saveEmployee="handleSaveEmployee" />
  </div>
</template>
<script lang='ts'>
...
export default defineComponent({
  async setup(props, context) {
    const useEmployeeHook = useEmployee()
    await useEmployeeHook.getEmployees() // get employees
    await new Promise((resolve) => {
      setTimeout(() => {
        resolve("done")
      }, 2000);
    })
    return useEmployeeHook
